...
const DeleteCourseModal = ({ open, onClose, courseId }: any) => {

    const onSubmit = () => {
        deleteCourse(courseId).finally(() => onClose())
    }
    ...
    return (
        <Dialog open={open} keepMounted onClose={onClose} fullWidth maxWidth="sm" scroll="paper">
            <DialogTitle>
                ...
                <div style={{ direction: 'rtl' }}>حذف دوره</div>
                <div></div>
            </DialogTitle>
            <DialogContent dividers>
                <Typography sx={{ width: 1, textAlign: 'center' }}>آیا از حذف دوره اطمینان دارید؟</Typography>
            </DialogContent>
            <DialogActions>
